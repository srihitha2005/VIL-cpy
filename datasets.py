...
def augment_dataset_same_class_mixup(dataset, num_augs=2, alpha=0.4):
    """
    For each sample in a class, generate `num_augs` MixUp images using samples from the same class.
    Returns a list of (augmented_img, label) pairs.
    """
    ...
    for label, images in tqdm.tqdm(class_data.items()):
        for img in images:
            augmented_dataset.append((img, label))  # original

            for _ in range(num_augs):
                img2 = random.choice(images)
                mixed_img = mixup_same_class(img, img2, alpha)
                augmented_dataset.append((mixed_img, label))  # same label
...
def build_continual_dataloader(args):
    ...
    if mode in ['til', 'cil']:
        ...
        if 'iDigits' in args.dataset:
            ...
            dataset_val.data = [
                augment_dataset_same_class_mixup(domain, num_augs=2, alpha=0.4)
                for domain in tqdm.tqdm(dataset_val.data)
            ]
